...
def limit_velocity(current_v: float, current_w: float, target_v: float, target_w: float,
                   ...
                   decel_max: float, angular_accel_max: float,
                   v_min: float = 0.0) -> tuple[float, float]:
    target_v = clamp(target_v, v_min, v_max)
    ...
    return (
        clamp(current_v + clamp(dv, -dv_limit, dv_limit), v_min, v_max),
        clamp(current_w + dw, -w_max, w_max),
    )
